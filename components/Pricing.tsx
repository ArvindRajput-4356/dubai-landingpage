...
const Pricing: React.FC = () => {
  return (
    <section id="pricing" className="py-24 bg-brand-blue">
      <div className="container mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto items-start">
          {tiers.map((tier, index) => (
            <div 
              key={index} 
              className={`relative p-8 rounded-2xl transition-all duration-300 h-full flex flex-col ${
                tier.recommended 
                  ? 'bg-slate-800 border-2 border-brand-gold shadow-2xl scale-105 z-10' 
                  : 'bg-slate-900 border border-slate-800 hover:border-slate-600'
              }`}
            >
              {tier.recommended && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-brand-gold text-slate-900 px-4 py-1 rounded-full text-xs font-bold uppercase">
                  Most Popular
                </div>
              )}
              ...
              <div className="text-3xl font-bold text-white mb-6">
                {tier.price} <span className="text-sm font-normal text-slate-500">{tier.price !== 'Custom' && '/month'}</span>
              </div>
              
              <ul className="space-y-4 mb-8 flex-grow">
                {tier.features.map((feature, i) => (
                  <li key={i} className="flex items-start gap-3 text-sm text-slate-300">
                    <Check className={`w-5 h-5 shrink-0 ${tier.recommended ? 'text-brand-gold' : 'text-blue-500'}`} />
                    {feature}
                  </li>
                ))}
              </ul>
